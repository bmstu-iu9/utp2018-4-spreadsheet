...
const removeUserData = (req, res) => {
    ...
    sendAuthRequest('/check_session', postAuthData).then(
        authINFO => {
            if (authINFO.error) {
                logs.log(`\x1b[34mREMOVE USER DATA\x1b[0m \x1b[31mFAILED\x1b[0m: User: ${cookies['token']}, Error: ${ERROR_MESSAGES[authINFO.error]}`);
            ...
            if (parsedURL.query['status'] == CONFIG.GUEST) { //string&int
                ...
                logs.log(`\x1b[34mREMOVE USER DATA\x1b[0m \x1b[31mFAILED\x1b[0m: User: ${cookies['token']} have no permission to load data.`);
            ...
            sendSaveRequest(adress, postSaveData).then(
                loadINFO => {
                    ...
                    logs.log(`\x1b[34mREMOVE USER DATA\x1b[0m \x1b[32mSUCCESS\x1b[0m: SessionID ${cookies['token']}, Title: ${parsedURL.query['title']}, Email: ${authINFO.email}`);
                    return returnJSON({
                        error: null
                    }, res);
                },
